...
assay = 'CCTE_Deisenroth_AIME_384WELL_LUC_Active'

# Split into SOURCE and TARGET by stalk-shape
# There are two stalk-shape values: "e" and "t"
target_df = pd.read_csv(f'Example_Data/ToxCastERa/{assay}-2026-04-30.csv', sep = ',', index_col=0)
...
for fold in range(5):
    ...
    with open(f'/home/lisa-marie-rolli/ToxCastBenchmark/model_inputs/steroidal/{assay}/fold{fold}/train.txt', 'r') as train_file:
        train_samples = train_file.read().splitlines()
    with open(f'/home/lisa-marie-rolli/ToxCastBenchmark/model_inputs/steroidal/{assay}/fold{fold}/test.txt', 'r') as test_file:
        test_samples = test_file.read().splitlines()
    with open(f'/home/lisa-marie-rolli/ToxCastBenchmark/model_inputs/steroidal/{assay}/fold{fold}/physchem_feature_names_mrmr.txt', 'r') as feature_file:
...
plot_mcc_boxplot(results_target_mcc, results_source_mcc, results_wma_mcc, results_transfer_strut_mcc, results_transfer_ser_mcc, filename='toxcast_ccte_active_mcc.png')
plot_mcc_boxplot(results_target_mae, results_source_mae, results_wma_mae, results_transfer_strut_mae, results_transfer_ser_mae, filename='toxcast_ccte_active_mae.png', ylabel='Mean Absolute Deviation (MAE)')